...
static OQS_STATUS example_heap(void) {
  ...
  uint8_t *coins = NULL;
	...
	if (kem == NULL) {
		printf("[example_heap]  OQS_KEM_alg_classic_mceliece_460896 was not enabled at "
		       "compile-time.\n");
		return OQS_ERROR;
	}
  ...
  printf("rc: %d\n", rc);
	...
	printf("[example_heap] OQS_KEM_alg_classic_mceliece_460896 operations completed.\n");
	cleanup_heap(secret_key, shared_secret_e, shared_secret_d, public_key,
	             ciphertext, kem, shared_secret_d2);
