...
int PQCLEAN_MCELIECE6960119_AVX_crypto_kem_enc(
    ...
    PQCLEAN_MCELIECE6960119_AVX_encrypt(c, coins, pk);
...
int PQCLEAN_MCELIECE6960119_AVX_crypto_kem_dec(
    ...
    ret_decrypt = (uint8_t)PQCLEAN_MCELIECE6960119_AVX_decrypt(e, sk + SYS_N / 8, c);
...
(
    ...
    while (1) {
        rp = r;
        PQCLEAN_MCELIECE6960119_AVX_aes256ctr(r, sizeof(r), nonce, seed);
        memcpy(seed, &r[ sizeof(r) - 32 ], 32);

        for (i = 0; i < SYS_T; i++) {
            f[i] = PQCLEAN_MCELIECE6960119_AVX_load2(rp + i * 2);
        }
        rp += sizeof(f);
        if (PQCLEAN_MCELIECE6960119_AVX_genpoly_gen(irr, f)) {
            continue;
        }

        for (i = 0; i < (1 << GFBITS); i++) {
            perm[i] = PQCLEAN_MCELIECE6960119_AVX_load4(rp + i * 4);
        }
        rp += sizeof(perm);
        if (PQCLEAN_MCELIECE6960119_AVX_perm_check(perm)) {
            continue;
        }

        for (i = 0; i < SYS_T;   i++) {
            PQCLEAN_MCELIECE6960119_AVX_store2(sk + SYS_N / 8 + i * 2, irr[i]);
        }
        if (PQCLEAN_MCELIECE6960119_AVX_pk_gen(pk, perm, sk + SYS_N / 8)) {
            continue;
        }

        memcpy(sk, rp, SYS_N / 8);
        PQCLEAN_MCELIECE6960119_AVX_controlbits(sk + SYS_N / 8 + IRR_BYTES, perm);
...
void PQCLEAN_MCELIECE6960119_AVX_crypto_kem_gen_e(uint8_t *e) {
  gen_e_6960119_avx(e);
}
