...
int PQCLEAN_MCELIECE348864F_AVX_crypto_kem_enc(
    uint8_t *c,
    uint8_t *key,
    const uint8_t *pk,
    const uint8_t *coins
    ...
    PQCLEAN_MCELIECE348864F_AVX_encrypt(c, coins, pk);
...
int PQCLEAN_MCELIECE348864F_AVX_crypto_kem_dec(
    uint8_t *key,
    const uint8_t *c,
    const uint8_t *sk
) {
    ...
    ret_decrypt = (uint8_t)PQCLEAN_MCELIECE348864F_AVX_decrypt(e, sk + SYS_N / 8, c);
...
(
    ...
    while (1) {
        rp = r;
        PQCLEAN_MCELIECE348864F_AVX_aes256ctr(r, sizeof(r), nonce, seed);
        memcpy(seed, &r[ sizeof(r) - 32 ], 32);

        for (i = 0; i < SYS_T; i++) {
            f[i] = PQCLEAN_MCELIECE348864F_AVX_load2(rp + i * 2);
        }
        rp += sizeof(f);
        if (PQCLEAN_MCELIECE348864F_AVX_genpoly_gen(irr, f)) {
            continue;
        }

        for (i = 0; i < (1 << GFBITS); i++) {
            perm[i] = PQCLEAN_MCELIECE348864F_AVX_load4(rp + i * 4);
        }
        rp += sizeof(perm);
        if (PQCLEAN_MCELIECE348864F_AVX_perm_check(perm)) {
            continue;
        }

        for (i = 0; i < SYS_T;   i++) {
            PQCLEAN_MCELIECE348864F_AVX_store2(sk + SYS_N / 8 + i * 2, irr[i]);
        }
        if (PQCLEAN_MCELIECE348864F_AVX_pk_gen(pk, perm, sk + SYS_N / 8)) {
            continue;
        }

        memcpy(sk, rp, SYS_N / 8);
        PQCLEAN_MCELIECE348864F_AVX_controlbits(sk + SYS_N / 8 + IRR_BYTES, perm);
...
void PQCLEAN_MCELIECE348864F_AVX_crypto_kem_gen_e(uint8_t *e) {
  gen_e_348864f_avx(e);
}
