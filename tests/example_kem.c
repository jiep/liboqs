...
static OQS_STATUS example_heap(void) {
  ...
  uint8_t *coins = NULL;
	...
	kem = OQS_KEM_new(OQS_KEM_alg_classic_mceliece_8192128);
	if (kem == NULL) {
		printf("[example_heap]  OQS_KEM_alg_classic_mceliece_8192128 was not enabled at "
		       "compile-time.\n");
		return OQS_ERROR;
	}
  ...
  print_hex(ciphertext2, kem->length_ciphertext);
	...
	printf("[example_heap] OQS_KEM_alg_classic_mceliece_8192128 operations completed.\n");
	cleanup_heap(secret_key, shared_secret_e, shared_secret_d, public_key,
	             ciphertext, kem, shared_secret_d2);
