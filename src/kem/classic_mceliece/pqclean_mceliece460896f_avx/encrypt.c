...
void gen_e_60896f_avx(unsigned char *e) {
    ...
    while (1) {
        ...
        PQCLEAN_MCELIECE460896F_AVX_int32_sort(ind32, SYS_T);
    ...
    for (i = 0; i < (SYS_N + 63) / 64; i++) {
        PQCLEAN_MCELIECE460896F_AVX_store8(e, e_int[i]);
        e += 8;
    }
}
...
void PQCLEAN_MCELIECE460896F_AVX_encrypt(unsigned char *s, const unsigned char *e, const unsigned char *pk) {
    // gen_e(e);
    PQCLEAN_MCELIECE460896F_AVX_syndrome_asm(s, pk, e);
}
