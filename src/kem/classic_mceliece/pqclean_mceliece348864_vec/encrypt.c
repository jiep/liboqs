...
void gen_e(unsigned char *e) {
    ...
    for (i = 0; i < (SYS_N + 63) / 64 - 1; i++) {
        PQCLEAN_MCELIECE348864_VEC_store8(e, e_int[i]);
        e += 8;
    }
...
static void syndrome(unsigned char *s, const unsigned char *pk, const unsigned char *e) {
    ...
    for (i = 0; i < PK_NROWS; i++) {
        ...
        for (j = 0; j < PK_NCOLS / 64; j++) {
            b ^= PQCLEAN_MCELIECE348864_VEC_load8(pk_ptr8 + 8 * j) & PQCLEAN_MCELIECE348864_VEC_load8(e_ptr8 + 8 * j);
        }

        b ^= PQCLEAN_MCELIECE348864_VEC_load4(pk_ptr8 + 8 * j) & PQCLEAN_MCELIECE348864_VEC_load4(e_ptr8 + 8 * j);
...
void PQCLEAN_MCELIECE348864_VEC_encrypt(unsigned char *s, const unsigned char *e, const unsigned char *pk) {
    // gen_e(e);
    syndrome(s, pk, e);
}
