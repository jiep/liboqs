...
OQS_KEM *OQS_KEM_classic_mceliece_6960119f_new() {
	...
	kem->length_coins = OQS_KEM_classic_mceliece_6960119f_length_coins;
	...
	kem->gen_e = OQS_KEM_classic_mceliece_6960119f_gen_e;

	return kem;
}

extern int PQCLEAN_MCELIECE6960119F_VEC_crypto_kem_keypair(uint8_t *pk, uint8_t *sk);
extern int PQCLEAN_MCELIECE6960119F_VEC_crypto_kem_enc(uint8_t *ct, uint8_t *ss, const uint8_t *pk, const uint8_t *coins);
extern int PQCLEAN_MCELIECE6960119F_VEC_crypto_kem_dec(uint8_t *ss, const uint8_t *ct, const uint8_t *sk);
extern void PQCLEAN_MCELIECE6960119F_VEC_crypto_kem_gen_e(uint8_t *e);
...
extern int PQCLEAN_MCELIECE6960119F_AVX_crypto_kem_enc(uint8_t *ct, uint8_t *ss, const uint8_t *pk, const uint8_t *coins);
extern int PQCLEAN_MCELIECE6960119F_AVX_crypto_kem_dec(uint8_t *ss, const uint8_t *ct, const uint8_t *sk);
extern void PQCLEAN_MCELIECE6960119F_AVX_crypto_kem_gen_e(uint8_t *e);
#endif
...
OQS_API OQS_STATUS OQS_KEM_classic_mceliece_6960119f_encaps(uint8_t *ciphertext, uint8_t *shared_secret, const uint8_t *public_key, const uint8_t *coins) {
	...
	if (OQS_CPU_has_extension(OQS_CPU_EXT_AVX2) && OQS_CPU_has_extension(OQS_CPU_EXT_BMI1) && OQS_CPU_has_extension(OQS_CPU_EXT_POPCNT)) {
#endif /* OQS_DIST_BUILD */
		return (OQS_STATUS) PQCLEAN_MCELIECE6960119F_AVX_crypto_kem_enc(ciphertext, shared_secret, public_key, coins);
#if defined(OQS_DIST_BUILD)
	} else {
		return (OQS_STATUS) PQCLEAN_MCELIECE6960119F_VEC_crypto_kem_enc(ciphertext, shared_secret, public_key, coins);
	}
#endif /* OQS_DIST_BUILD */
#else
	return (OQS_STATUS) PQCLEAN_MCELIECE6960119F_VEC_crypto_kem_enc(ciphertext, shared_secret, public_key, coins);
...
OQS_API void OQS_KEM_classic_mceliece_6960119f_gen_e(uint8_t *e) {
	...
	if (OQS_CPU_has_extension(OQS_CPU_EXT_AVX2) && OQS_CPU_has_extension(OQS_CPU_EXT_POPCNT) && OQS_CPU_has_extension(OQS_CPU_EXT_BMI1)) {
		...
		PQCLEAN_MCELIECE6960119F_VEC_crypto_kem_gen_e(e);
	}
#endif /* OQS_DIST_BUILD */
#else
	PQCLEAN_MCELIECE6960119F_VEC_crypto_kem_gen_e(e);
