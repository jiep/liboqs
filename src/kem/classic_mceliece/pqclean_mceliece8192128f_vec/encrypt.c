...
void gen_e_8192128f(unsigned char *e) {
    size_t i, j;
    int eq;
    ...
    for (i = 0; i < SYS_N / 64; i++) {
        PQCLEAN_MCELIECE8192128F_VEC_store8(e + i * 8, e_int[i]);
    }
}
...
static void syndrome(unsigned char *s, const unsigned char *pk, const unsigned char *e) {
    ...
    for (i = 0; i < PK_NROWS; i++) {
        ...
        for (j = 0; j < PK_NCOLS / 64; j++) {
            b ^= PQCLEAN_MCELIECE8192128F_VEC_load8(pk_ptr8 + 8 * j) & PQCLEAN_MCELIECE8192128F_VEC_load8(e_ptr8 + 8 * j);
        }
...
void PQCLEAN_MCELIECE8192128F_VEC_encrypt(unsigned char *s, const unsigned char *e, const unsigned char *pk) {
    // gen_e(e);
    syndrome(s, pk, e);
}
